    ...
    .then(registration => {
      ...
      registration.onupdatefound = () => {
        ...
        installingWorker.onstatechange = () => {
          if (installingWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              ...
              console.log(logPrefix, 'Content is cached for offline use.');
        ...
        installingWorker.onerror = error => {
          console.error(logPrefix, 'Installing worker:', error);
        };
      };
    })
    .catch(error => {
...
export function register(config?: Config): void {
  if (appEnv.prod && 'serviceWorker' in navigator) {
    ...
    const publicUrl = new URL(appConfig.client.output.publicPath, window.location.href);
    if (publicUrl.origin !== window.location.origin) {
      // Our service worker won't work if PUBLIC_URL is on a different origin
      // from what our page is served on. This might happen if a CDN is used to
      // serve assets; see https://github.com/facebook/create-react-app/issues/2374
      console.warn(logPrefix, 'Origins are different.');
      return;
    }
